...
/**
 * This is the model class for table "{{%dedicated_transport_truck}}".
 *
 * @property int $dt_truck_id ID
 * @property int $user_id Покупатель
 * @property int $semi_trailer_type Тип полуприцепа
 * @property int $tent_hard_board Жесткий борт (тентовый)
 * @property int $tent_removable_top_beam Съемная верхняя балка (тентовый)
 * @property int $tent_removable_side_beam Съемная боковая балка (тентовый)
 ...
 */
class DedicatedTransportTruck extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return '{{%dedicated_transport_truck}}';
    }
    ...
    public function rules()
    {
        return [
            ...
            [['semi_trailer_type', 'cargo_params', 'hazard_class', 'declared_price', 'from', 'to', 'pick_up_date'], 'required'],
            ['semi_trailer_type', 'in', 'range' => [1, 2]],
            [['dt_truck_id', 'user_id', 'tent_hard_board', 'tent_removable_top_beam', 'tent_removable_side_beam', 'hazard_class', 'city_pick_up', 'city_delivery', 'from_address_loading', 'loading_operations', 'territory_entry', 'filling', 'hard_package', 'pallet_transparent', 'pallet_black', 'tent_remove_to', 'tent_remove_from', 'pallet_board_pack', 'is_draft'], 'integer'],
    ...
    public function attributeLabels()
    {
        return [
            'dt_truck_id' => 'ID',
            'user_id' => 'Покупатель',
            'semi_trailer_type' => 'Тип полуприцепа',
            'tent_hard_board' => 'Жесткий борт',
            'tent_removable_top_beam' => 'Съемная верхняя балка',
            'tent_removable_side_beam' => 'Съемная боковая балка',
    ...
    public function send_email($email, $role) {
        ...
        return $mailer->compose(
                ['html' => \Yii::$app->params['email'][$role]['dedicated_truck']['template']],
                ['model' => $this, 'user' => $user, 'contacts' => $contacts]
            )
            ->setTo($email)
            ->setFrom([\Yii::$app->params['fromEmail'] => \Yii::$app->params['fromName']])
            ->setSubject(\Yii::$app->params['email'][$role]['dedicated_truck']['subject'])
        ->send();
    }
