...
<div class="general-cargo-letter-form">

    <?php $form = ActiveForm::begin(); ?>

    <?php
        echo '<br>Пользователь: <b>'.$model->order->user->username.'</b><br>';
        echo '<br>Дата создания: <b>'.date('d.m.Y', $model->order->create_date).'</b><br><br>';
          //$form->field($model, 'user_id')->dropDownList(ArrayHelper::map(User::findAll(['role' => 1]), 'id', 'username'));
  ?>
    <?php
    //      echo '<br>Расстояние: '. $model->order->route_length.'<br>';
      //    echo '<br>Расчетная цена доставки: '. $model->order->calculated_price.'<br><br>';
    ?>
