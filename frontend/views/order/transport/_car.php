...
<div class="load-capacity row no-gutters">
    <div class="col-12">
        <div class="margin"></div>
    </div>
    <div class="label">
        <label><?= $model->getAttributeLabel('load_capacity') ?>:</label>
    </div>
    <div class="col-md-3">
        <?php
            if (!$model->load_capacity) {
                $model->load_capacity = 1;
            }
        ?>
        <?=
            $form->field($model,'load_capacity')->radioList(\Yii::$app->params['load_capacity'], [
               'separator'=>"<div></div>",
               'encode'=>'html',
               'id' => 'load-capacity-radio',
